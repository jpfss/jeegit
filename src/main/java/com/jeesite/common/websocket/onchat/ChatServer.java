package com.jeesite.common.websocket.onchat;
...
import com.jeesite.common.json.AjaxJson;
import com.jeesite.common.utils.SpringContextHolder;
import com.jeesite.common.websocket.utils.Constant;
...
import com.jeesite.modules.iim.service.ChatHistoryService;
		...
		if (null != message && message.contains(Constant._msg_)) {// 发送消息
			...
			if ("group".equals(type)) {// 如果是群聊
				...
				for (LayGroupUser lgUser : layGroupUserlist) {
					...
					chat.setType("group");
					if (toUserConn != null) {
						message = groupId + Constant._msg_ + lgUser.getUser().getLoginName() + Constant._msg_ + msg
								+ Constant._msg_ + avatar + Constant._msg_ + type + Constant._msg_ + fromUsername
								+ Constant._msg_ + datatime;
